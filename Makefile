...
ifdef DOCKER_PUSH
	$(eval IMAGE_ID = $(shell $(SKOPEO) inspect --raw docker-archive:$< | $(JQ) .config.digest))
	$(PODMAN) image push $(IMAGE_ID) $(TARGET_IMAGE):$$($(SKOPEO) inspect --config --raw docker-archive:$< | $(JQ) .config.Labels.version)
